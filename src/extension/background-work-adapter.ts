...
export async function executeDetachableSubagent(input:{pi:ExtensionAPI;id:string;params:{agent?:unknown;tasks?:unknown[];chain?:unknown[]};signal:AbortSignal;onUpdate?:((r:AgentToolResult<Details>)=>void);ctx:ExtensionContext;execute:(signal:AbortSignal,onUpdate:(r:AgentToolResult<Details>)=>void)=>Promise<AgentToolResult<Details>>}):Promise<AgentToolResult<Details>> {
 ...
 if(winner.type==="promoted")return{content:[{type:"text",text:`Backgrounded top-level subagent run as ${jobId} at the user's request. Its results will be delivered to you automatically in a later message when it finishes — do not wait, poll, or check status on it. Finish any unrelated work and end your turn so the user can keep chatting.`}],details:{mode:"management",results:[],backgroundWork:{jobId,state:"background"}}};
